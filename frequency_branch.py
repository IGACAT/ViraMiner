...
parser.add_argument("--input_path",
                    default="data/NN_data_clustered")  # data location
parser.add_argument("--filter_size", type=int,
                    default=5)  # with this you can scan other filter sizes
...
print("train_set_size: ", train_set_size, file=sys.stderr)
sys.stderr.flush()
...
print("Read validation data: {}".format(val_set_size), file=sys.stderr)
sys.stderr.flush()
...
print("Read test data: {}".format(test_set_size), file=sys.stderr)
sys.stderr.flush()
...
print("Converted to numpy arrays.", file=sys.stderr)
sys.stderr.flush()
...
first_freq = Conv1D(args.layer_sizes, args.filter_size,
                    activation="relu")(inputs)
freq_pooling = GlobalAveragePooling1D()(
    first_freq)  #returns one value per filter
drop_freq = Dropout(args.dropout)(freq_pooling)

fc_layer1 = Dense(args.layer_sizes, activation="relu",
                  name="freq_fc_layer1")(drop_freq)
...
callbacks.append(
    ModelCheckpointAUROC(filepath=args.save_path + ".hdf5",
