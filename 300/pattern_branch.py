...
parser.add_argument("--filter_size", type=int,
                    default=10)  # with this you can scan other filter sizes
...
print("Finished making numpy arrays.", file=sys.stderr)
sys.stderr.flush()
...
inputs = Input(shape=(
    sequence_length,
    5))  # 5 because we allow "ATGCN", even though there is no N-s in our data

first_conv = Conv1D(args.layer_sizes, args.filter_size,
