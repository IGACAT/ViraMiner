...
parser.add_argument(
    "--freq_model",
    default="oct2018/best_frequency.hdf5")  # pretrained frequency model
parser.add_argument(
    "--pattern_model",
    default="oct2018/best_pattern.hdf5")  # pretrained pattern model
...
print("Validation set size: {}".format(val_set_size), file=sys.stderr)
sys.stderr.flush()
...
print("Test set size".format(test_set_size), file=sys.stderr)
sys.stderr.flush()
...
inputs = Input(
    shape=(sequence_length,
           5))  # "None" means any sequence length, "5" because we have "ATGCN"
...
freq_model_cropped = Model(freq_model.inputs, freq_model.layers[-3].output
                           )  # crop layers -1 and -2, i.e sigmoid and dropout
pattern_model_cropped = Model(pattern_model.inputs,
                              pattern_model.layers[-3].output)
...
callbacks.append(
    ModelCheckpointAUROC(filepath=args.save_path + "_beforeFT.hdf5",
                         validation_data=(val_seqs, val_labels),
                         verbose=1,
                         save_best_only=True))
callbacks.append(
    EarlyStoppingAUROC(validation_data=(val_seqs, val_labels),
                       patience=6,
                       min_delta=0.001))
callbacks.append(roc_callback(
    ((val_seqs, val_labels))))  #defined in helper_with_N
...
model.fit_generator(generate_batches_from_file(args.input_path + "_train.csv",
                                               args.batch_size),
                    steps_per_epoch=tr_steps_per_ep,
                    epochs=args.epochs,
                    workers=1,
...
if args.finetuning == "True":
    ...
    callbacks = []
    callbacks.append(
        ModelCheckpointAUROC(filepath=args.save_path + "_afterFT.hdf5",
                             validation_data=(val_seqs, val_labels),
                             verbose=1,
                             save_best_only=True))
    callbacks.append(
    ...
    # This fitting procedure reads datapoints from file on as-needed basis
    model.fit_generator(generate_batches_from_file(
        args.input_path + "_train.csv", args.batch_size),
                        steps_per_epoch=tr_steps_per_ep,
                        epochs=args.epochs,
                        workers=1,
                        use_multiprocessing=False,
...
pred_probas = model.predict_generator(generate_batches_from_file(
    args.input_path + "_test.csv", args.batch_size),
...
pred_probas = model.predict_generator(generate_batches_from_file(
    args.input_path + "_validation.csv", args.batch_size),
                                      steps=val_steps_per_ep + 1,
                                      workers=1,
                                      use_multiprocessing=False)
pred_probas = pred_probas[:len(val_labels), :]
print("VAL ROC area under the curve \n", roc_auc_score(val_labels,
                                                       pred_probas))
