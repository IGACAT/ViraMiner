...
options_onehot = defaultdict(lambda: [0, 0, 0, 0, 1])

options_onehot['A'] = [1, 0, 0, 0, 0]
options_onehot['C'] = [0, 1, 0, 0, 0]
options_onehot['G'] = [0, 0, 1, 0, 0]
options_onehot['T'] = [0, 0, 0, 1, 0]
...
def DNA_to_onehot(dna_line):
    try:
        ...
        print(dna_line, file=sys.stdout)
        raise KeyError
    onehot_data = np.array(onehot_data)
    return onehot_data
...
def predict_one_by_one(model, seqs, labels):
    prediction_data = np.empty([len(seqs), 4
                                ])  # true_class, pred_class, proba, seq_length
...
def generate_batches_from_file(path, batch_size):
    ...
    submitted_counter = 0
    last_seq = ""
    print("generator uses b_size: ", batch_size, file=sys.stderr)
    sys.stderr.flush()
    ...
    while 1:
        ...
        print("opened file again", file=sys.stderr)
        sys.stderr.flush()
        ...
        for line in f:
            ...
            if batch_counter == batch_size:
                ...
                seqs = []
                labels = []
        print("Did all lines in file ",
              submitted_counter,
              total_counter,
              for_counter,
              file=sys.stderr)
        sys.stderr.flush()
        print("Unique project names", PROJECT_NAMES, file=sys.stderr)
        sys.stderr.flush()
...
class roc_callback(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if roc_val == 0.5:  #happens if learning has crashed
            ...
            print(
                "ATTENTION: Stopped learning process, becuase learning had in all probability crashed!"
            )
        return
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if self.epochs_since_last_save >= self.period:
            ...
            if self.save_best_only:
                ...
                if current is None:
                    warnings.warn(
                        'Can save best model. Current area_under_ROC is None.'
